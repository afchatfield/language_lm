#!/usr/bin/env python
"""Build and freeze the Spanish overcorrection set.

The Spanish counterpart of `build_overcorrection_set.py` -- see that script's
docstring for what the set is for. Same shape of output, drawn from
`spa_news_2023_100K` + `spa_wikipedia_2021_100K` instead of the German Leipzig
pull, tokenised with `errant_es`.

    python scripts/build_overcorrection_set_es.py [--size 400] [--refreeze]
"""
...
def main() -> None:
    settings = load_config("phase1_es")["overcorrection"]
    ...
    sentences = overcorrection_es.build(settings["corpora"], args.size, args.seed)

    path = overcorrection_es.OVERCORRECTION_DIR / "es.m2"
    ...
    if args.refreeze:
        _drop_entry(overcorrection_es.CORPUS)
    try:
        freeze_splits(
            corpus=overcorrection_es.CORPUS,
            split_paths={"all": path},
            source_url="Leipzig Corpora Collection; see configs/phase1_es.yaml",
        )
        print(f"Froze {overcorrection_es.CORPUS} in {SPLIT_MANIFEST}")
    except ManifestError as exc:
        print(f"Already frozen: {exc}")
    verify_manifest(corpora=[overcorrection_es.CORPUS])
