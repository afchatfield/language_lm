...
The bar is the best Phase 1 baseline, **F0.5 = 0.7019** (few-shot), not LanguageTool's 0.4134. This model **beats** it.

## Detection versus correction

| | Detection F0.5 | Correction F0.5 | Gap |
|---|---:|---:|---:|
| iter4-cw025 | 0.7480 | 0.6295 | +0.1185 |
...
|---|---:|---:|
| strict | 0.7072 | 0.6295 |
| lenient | 0.7125 | 0.6339 |

The headline stays the strict number. The lenient one is worth +0.0053, which is the answer to whether the detection-correction gap above is the annotator being fussy: it is not. Almost all of that gap is the model finding the right word to change and then changing it wrongly -- most often a misspelling whose intended word is recoverable, answered with a different word (`rabben` corrected to `finden`, not `rauben`).
...
| | Value | Reads as |
|---|---:|---|
| Coverage | 0.8620 | of 5,658 rewrites it made, 4,877 were listed |
| Precision | 0.9269 | of 5,701 changes it listed, 5,284 match a real rewrite |
| Type accuracy | 0.6955 | of the 5,284 that match, 3,675 carry the type ERRANT assigns |
...
0 with no readable correction, 3 cut off but recovered, 992 left unchanged, out of 2,903 answers.
