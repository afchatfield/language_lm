...
|---|---:|---:|---:|
| iter5-test | 0.7418 | 0.6210 | +0.1208 |
...
|---|---:|---:|
| strict | 0.7071 | 0.6210 |
| lenient | 0.7109 | 0.6250 |

The headline stays the strict number. The lenient one is worth +0.0037, which is the answer to whether the detection-correction gap above is the annotator being fussy: it is not. Almost all of that gap is the model finding the right word to change and then changing it wrongly -- most often a misspelling whose intended word is recoverable, answered with a different word (`rabben` corrected to `finden`, not `rauben`).
...
| | Value | Reads as |
|---|---:|---|
| Coverage | 0.8921 | of 5,337 rewrites it made, 4,761 were listed |
| Precision | 0.9453 | of 5,414 changes it listed, 5,118 match a real rewrite |
| Type accuracy | 0.7341 | of the 5,118 that match, 3,757 carry the type ERRANT assigns |
...
0 with no readable correction, 2 cut off but recovered, 946 left unchanged, out of 2,737 answers.
...
```json
{"correction": "Es kann sein , dass sie den Arbeitplatz verloren haben oder keine Familie mehr haben .", "changes": [{"was": "oder haben", "now": "oder haben", "type": "R:VERB:FORM"}, {"was": "haben keine", "now": "haben keine", "type": "R:DET:FORM"}, {"was": "mehr .", "now": "mehr haben .", "type": "M:AUX"}]}
```
