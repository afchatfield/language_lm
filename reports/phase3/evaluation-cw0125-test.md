...
The bar is the best Phase 1 baseline, **F0.5 = 0.7072** (few-shot), not LanguageTool's 0.4134. This model **beats** it.

## Detection versus correction

| | Detection F0.5 | Correction F0.5 | Gap |
|---|---:|---:|---:|
| cw0125-test | 0.7421 | 0.6251 | +0.1170 |
...
|---|---:|---:|
| strict | 0.7083 | 0.6251 |
| lenient | 0.7109 | 0.6286 |

The headline stays the strict number. The lenient one is worth +0.0026, which is the answer to whether the detection-correction gap above is the annotator being fussy: it is not. Almost all of that gap is the model finding the right word to change and then changing it wrongly -- most often a misspelling whose intended word is recoverable, answered with a different word (`rabben` corrected to `finden`, not `rauben`).
...
| | Value | Reads as |
|---|---:|---|
| Coverage | 0.8583 | of 5,330 rewrites it made, 4,575 were listed |
| Precision | 0.9170 | of 5,375 changes it listed, 4,929 match a real rewrite |
| Type accuracy | 0.7117 | of the 4,929 that match, 3,508 carry the type ERRANT assigns |
...
0 with no readable correction, 4 cut off but recovered, 944 left unchanged, out of 2,737 answers.
...
```json
{"correction": "Welche Lösung ist aber möglich , um diese Diskrepanz zwischen Studium und Arbeit , Gesellschaft und Studenten abzureißen ?", "changes": [{"was": "abzuwischen", "now": "abzureißen", "type": "R:VERB"}]}
```

```json
{"correction": "Es kann sein , dass sie den Arbeitplatz verloren haben oder keine Familie mehr haben .", "changes": [{"was": ", oder", "now": "oder", "type": "U:PUNCT"}, {"was": "haben keine", "now": "haben keine", "type": "R:SPELL"}, {"was": "mehr .", "now": "mehr haben .", "type": "M:AUX"}]}
```
