...
|---|---:|---:|---:|
| iter5-dpo-test | 0.7221 | 0.6019 | +0.1202 |
...
|---|---:|---:|
| strict | 0.6873 | 0.6019 |
| lenient | 0.6917 | 0.6065 |

The headline stays the strict number. The lenient one is worth +0.0044, which is the answer to whether the detection-correction gap above is the annotator being fussy: it is not. Almost all of that gap is the model finding the right word to change and then changing it wrongly -- most often a misspelling whose intended word is recoverable, answered with a different word (`rabben` corrected to `finden`, not `rauben`).
...
| | Value | Reads as |
|---|---:|---|
| Coverage | 0.8832 | of 5,533 rewrites it made, 4,887 were listed |
| Precision | 0.9384 | of 5,681 changes it listed, 5,331 match a real rewrite |
| Type accuracy | 0.7229 | of the 5,331 that match, 3,854 carry the type ERRANT assigns |
...
0 with no readable correction, 4 cut off but recovered, 895 left unchanged, out of 2,737 answers.
...
```json
{"correction": "Welche Lösung ist aber möglich , um diese Diskrepanz zwischen Studium und Arbeit , Gesellschaft und Studenten abzuwischen ?", "changes": []}
```

```json
{"correction": "Es kann sein , dass sie den Arbeitplatz verloren haben oder keine Familie mehr haben .", "changes": [{"was": ", oder", "now": "oder keine Familie mehr", "type": "M:OTHER"}, {"was": "mehr .", "now": "mehr haben .", "type": "M:AUX"}]}
```

```json
{"correction": "Ich werde hauptsächlich argumentieren , dass Kriminalität sich nicht auszahlt , aber ich werde jedoch auf positive und negative Fälle von Kriminalität hinweisen .", "changes": [{"was": "jedoch werde ich", "now": "ich werde jedoch", "type": "R:WO"}]}
```
...
```json
{"correction": "Sowohl Universitäten , die öffentliche Verwaltung als auch Medienagenturen brauchen Mitarbeiter , die im Internet Informationen suchen können , vermitteln können und eine gute Idee verkaufen können .", "changes": []}
```
